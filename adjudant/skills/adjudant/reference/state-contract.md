...
| `~/.claude/statusline-cache/ops-{key}`, line one `<unix ts> <message>`, line two `seen <ts>` | the ops flash: the whole bar becomes `⚡ <message>`. Line one is written by `scripts/_ops_flash.py` and nothing else (the hooks, the verbs and `hooks/scripts/_ops_flash.sh` all go through it); line two is stamped by the bar on the first paint, and the flash stays ten seconds from that stamp, never from the write. A newer flash rewrites the file without a `seen` line, which restarts the clock; one left unseen for ten minutes is dropped, not shown late. `{key}` is the session root, absolute with symlinks resolved, folded to the main checkout inside a linked worktree (the `gitdir:` pointer above), then `/` to `_`, space to `-`, last 120 characters |
| `$TMPDIR/claude-ultracode-{session_id}`, file exists | ultracode is on: the context bar paints in the ULTRA purple and a filled pip follows the effort code. Written by `hooks/scripts/user-prompt-reminder.sh` when a prompt contains the word `ultracode`, removed when one says `ultracode off`; the harness itself reports the level as a plain `xhigh` |
