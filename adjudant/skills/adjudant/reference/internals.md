...
| Stop | `hooks/scripts/stop-canary.py` | The drift canary. SessionStart states one rule (end every message with a rare codeword) and never restates it; this reads `last_assistant_message` and records a hit or a miss in `$TMPDIR/adjudant-canary-{session_id}.json`, with `streak` and `max_streak` for consecutive misses — one stray miss is a tool-heavy turn, a run of them is drift. It emits nothing on any path, and no hook or verb reads the file back. That is deliberate: a reading the model can see is a reading the model acts on, and it did act — the earlier block and per-turn report had it winding the session down before the user had decided anything. Coercing compliance also manufactures the appearance of health in the one measurement built to detect its absence. Read the file yourself when you want the tally; no vault writes |
